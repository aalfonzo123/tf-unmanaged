resource "google_compute_address" "public-address-1" {
  name         = "public-address-1"
  address_type = "EXTERNAL"
  region       = var.network.region
}

resource "google_compute_instance" "backend-instance-1" {
  ...
  network_interface {
    ...
    access_config {
      nat_ip = google_compute_address.public-address-1.address
    }
  }
